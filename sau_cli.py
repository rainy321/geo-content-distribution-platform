...
class BilibiliVideoUploadRequest:
    ...
    creation_statement: int | None = None
...
async def upload_bilibili_video(request: BilibiliVideoUploadRequest) -> Path:
    ...
    if request.creation_statement is not None:
        import json

        if request.creation_statement not in (-1, 1, 2, 3, 4):
            raise RuntimeError(
                f"Invalid creation_statement: {request.creation_statement}. "
                "Allowed: -1(无需标注), 1(AI), 2(虚构演绎), 3(营销), 4(个人观点)"
            )
        extra_fields = json.dumps(
            {"creation_statement": {"id": int(request.creation_statement)}},
            ensure_ascii=False,
            separators=(",", ":"),
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    upload_video_parser.add_argument("--title", default="", help="Video title (optional for Douyin)")
    ...
    kuaishou_upload_video_parser.add_argument("--title", default="", help="Video title (optional for Kuaishou)")
    ...
    xiaohongshu_upload_video_parser.add_argument("--title", default="", help="Video title (optional for Xiaohongshu)")
    ...
    xiaohongshu_upload_note_parser.add_argument("--title", default="", help="Note title (optional for Xiaohongshu)")
    ...
    bilibili_upload_video_parser.add_argument(
        ...
        choices=[-1, 1, 2, 3, 4],
        default=None,
        help="Bilibili creation statement id: -1 none, 1 AI, 2 fiction, 3 marketing, 4 opinion",
    )
    ...
    if args.platform == "bilibili":
        ...
        if args.action == "upload-video":
            request = BilibiliVideoUploadRequest(
                ...
                tags=parse_tags(args.tags),
                publish_date=args.schedule or 0,
                creation_statement=args.creation_statement,
            )
