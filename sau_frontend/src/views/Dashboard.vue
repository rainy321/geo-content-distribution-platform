          ...
          <el-card class="stat-card">
            ...
            <div class="stat-footer">
              <div class="stat-detail platform-tags">
                <el-tooltip
                  v-for="item in platformStats.breakdown"
                  :key="item.name"
...
// 平台统计：覆盖账号管理中的全部平台类型（含百家号/B站/头条/搜狐/知乎等）
const platformTagTypes = ['success', 'danger', 'warning', 'info', '', 'success', 'danger', 'warning', 'info']
const platformStats = computed(() => {
  const accounts = accountStore.accounts || []
  const byType = {}
  for (const a of accounts) {
    const name = (a.platform && a.platform !== '未知')
      ? a.platform
      : (PLATFORM_TYPES[a.type] || null)
    if (!name) continue
    byType[name] = (byType[name] || 0) + 1
  }
  const breakdown = Object.values(PLATFORM_TYPES).map((name, index) => ({
    name,
    count: byType[name] || 0,
    tagType: platformTagTypes[index % platformTagTypes.length]
  }))
  const total = breakdown.filter(item => item.count > 0).length
