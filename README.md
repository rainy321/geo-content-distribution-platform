...
| `ALLOW_REAL_PUBLISHING` | `false` | 真实发布总开关；当前接入知乎、今日头条与搜狐号文章 |
...
知乎、今日头条和搜狐号真实发布默认关闭。只有同时满足 `DEMO_MODE=false`、`ALLOW_REAL_PUBLISHING=true`、任务本身不是 Demo，并且操作者在发布中心二次确认时才会进入真实适配器。三个适配器都会先验证登录状态。知乎发布前通过当前账号的公开文章列表精确匹配标题，已存在时直接返回原链接；发布后最多轮询 3 次公开文章结果并自动对账，不会自动重复点击“发布”。今日头条和搜狐号都只在平台返回公开文章链接时标记成功；只有提交证据或发生超时但最终状态未知时保持 `processing`，要求先到平台后台核对，避免盲目重试造成重复文章。搜狐号会声明“包含AI创作内容”，并支持无封面文章；平台明确提示未实名禁止发文时，会在填写内容前停止并进入“待人工确认”。扫码、验证码、风控或 Cookie 失效同样会进入“待人工确认”，系统不会绕过平台安全机制。百家号和小红书在 GEO 任务流中仍只支持 Demo 任务，不能伪装成真实发布成功。
