...
| --- | --- | --- | --- | --- |
| 抖音 | 图片笔记 | 图片 | `DouYinNote` | 两次最终点击后平台无记录；等待入库并需新授权 |
| 快手 | 图片笔记 | 图片 | `KSNote` | 两次最终点击后平台无记录；等待入库并需新授权 |
| Bilibili | 视频 | 视频 | `biliup` | 两次 `601` 限流、一次上传锁；内容管理为空 |
| 视频号 | 视频 | 视频 | `TencentVideo` | 创作者中心已出现精确标题与发布时间；公开链接待取 |
| TikTok | 视频 | 视频 | `TiktokVideo` | 任务 `26` 已上传并单击 Post，状态未知；后台暂未见标题 |
...
本轮合规视频为本地 `videoFile/geo-p2-neutral-test.mp4`：10 秒、720×1280、H.264、无音频的中性几何动画，不含人物、音乐、平台 Logo、品牌或效果承诺。视频号已取得创作者后台发布证据；TikTok 任务 `26` 已完成上传并单击 Post 但未取得内容记录；其他平台的真实结果和继续条件以 `docs/GEO-MVP-ACCEPTANCE.md` 未完成项账本为准。
...
- 本次已在项目所有者登录并明确授权后执行 P2 多轮真实验收，但只有视频号取得创作者后台发布证据；抖音、快手、Bilibili、TikTok 均没有可核验的内容记录，不能把技术尝试写成发布成功。
